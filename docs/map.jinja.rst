...
We will see a quick setup to configure the ``nginx`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``nginx`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``nginx`` formulas under ``/srv/salt/nginx/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/nginx/parameters/dns:domain/
    ln -s dns:domain /srv/salt/nginx/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/nginx/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/nginx/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``nginx`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``nginx`` formulas under ``/srv/salt/nginx/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/nginx/parameters/roles
...
We create a configuration for the role ``nginx/server`` in ``/srv/salt/nginx/parameters/roles/nginx/server.yaml``:
...
We create another configuration for the role ``nginx/client`` in ``/srv/salt/nginx/parameters/roles/nginx/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles nginx/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles nginx/client
...
.. code-block:: console
    ...
    mv /srv/salt/nginx/parameters/dns:domain/  '/srv/salt/nginx/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-nginx-as-json:
      test.nop:
        - name: {{ nginx | json }}
...
Here is an example based on `template-formula/nginx/config/file.sls`_:
...
This ``sls`` file expose a ``nginx`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ nginx['winner'] }}
...
.. _template-formula/nginx/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/nginx/config/file.sls
